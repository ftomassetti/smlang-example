...
class MappingTest {
    ...
    @test fun mapSimpleFileWithoutPositions() {
        ...
        val expectedAst = MiniCalcFile(listOf(
    ...
    @test fun mapSimpleFileWithPositions() {
        val code = """var a = 1 + 2
                     |a = 7 * (2 / 3)""".trimMargin("|")
        val ast = MiniCalcAntlrParserFacade.parse(code).root!!.toAst(considerPosition = true)
        val expectedAst = MiniCalcFile(listOf(
                VarDeclaration("a",
                        SumExpression(
    ...
    @test fun mapCastInt() {
        val code = "a = 7 as Int"
        val ast = MiniCalcAntlrParserFacade.parse(code).root!!.toAst()
        val expectedAst = MiniCalcFile(listOf(Assignment("a", TypeConversion(IntLit("7"), IntType()))))
        assertEquals(expectedAst, ast)
    }
    ...
    @test fun mapCastDecimal() {
        ...
        val ast = MiniCalcAntlrParserFacade.parse(code).root!!.toAst()
        val expectedAst = MiniCalcFile(listOf(Assignment("a", TypeConversion(IntLit("7"), DecimalType()))))
        assertEquals(expectedAst, ast)
    }
